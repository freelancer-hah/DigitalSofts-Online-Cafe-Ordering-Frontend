...
const RiderDashboard = () => {
  ...
  // Fetch data when rider is confirmed
  useEffect(() => {
    if (isRider) {
      fetchData();
      // Connect to socket
  ...
  const fetchData = async () => {
    try {
      ...
      if (active) setCurrentDeliveryId(active._id);
    } catch (error) {
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4 mb-6">
        <div className="flex items-center gap-4 flex-wrap">
          ...
          <div className="flex gap-2">
            <button onClick={() => updateStatus('online')} className="bg-green-500 text-white px-4 py-1 rounded-lg hover:bg-green-600 transition">Go Online</button>
            <button onClick={() => updateStatus('offline')} className="bg-gray-500 text-white px-4 py-1 rounded-lg hover:bg-gray-600 transition">Go Offline</button>
          </div>
        </div>
      </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4 mb-6">
          ...
          {assignedOrders.map(delivery => (
            <div key={delivery._id} className="flex items-center justify-between p-3 bg-yellow-50 rounded-lg border border-yellow-200 mb-2">
              ...
              <button onClick={() => acceptDelivery(delivery._id)} className="bg-green-500 text-white px-4 py-2 rounded-lg hover:bg-green-600 transition">Accept</button>
            </div>
          ))}
        </div>
      )}
              ...
              <div className="flex justify-between items-center">
                ...
                <div className="flex gap-2">
                  {delivery.status === 'accepted' && (
                    <button onClick={() => updateDeliveryStatus(delivery._id, 'pickup')} className="bg-blue-500 text-white px-3 py-1 rounded-lg hover:bg-blue-600 transition text-sm">Picked Up</button>
                  )}
                  {delivery.status === 'picked_up' && (
                    <button onClick={() => updateDeliveryStatus(delivery._id, 'start')} className="bg-purple-500 text-white px-3 py-1 rounded-lg hover:bg-purple-600 transition text-sm">Start Delivery</button>
                  )}
                  {delivery.status === 'on_way' && (
                    <button onClick={() => updateDeliveryStatus(delivery._id, 'complete')} className="bg-green-500 text-white px-3 py-1 rounded-lg hover:bg-green-600 transition text-sm">Delivered</button>
                  )}
                </div>
              </div>
              {delivery.status === 'on_way' && (
                <div className="mt-2 text-xs text-gray-500 flex items-center gap-2">
                  ...
                  Live location: {location.lat.toFixed(6)}, {location.lng.toFixed(6)}
                </div>
              )}
            </div>
          ))
